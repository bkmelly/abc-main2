...
btn.onclick = () => {
    let remov = document.getElementById("search-bar");
...
const aboutForm = document.getElementById('footer-sub2');
const newSuccessMessage = document.getElementById('new-success-box'); // New popup

if (aboutForm && newSuccessMessage) {
    aboutForm.addEventListener('submit', (event) => {
        event.preventDefault();
        console.log("Form submitted"); // Debug message
        newSuccessMessage.innerHTML = 'You Subscribe successfully!';
    ...
    console.error("Form or new success message element not found");
}

// form submission
$(document).ready(function() {
    $('#footer-sub2').on('submit', function(event) {
        event.preventDefault();
        ...
        $.ajax({
            url: '/subscribe', // Make sure this matches your server route
            type: 'POST',
            data: formData,
            success: function(response) {
                console.log("AJAX Success:", response); // Log success response
                $('#new-success-box').html('<p>Subscription successful!</p>'); // Ensure correct ID
                $('#new-success-box').show(); // Show success message
                $('#footer-sub2')[0].reset(); // Reset the form
                setTimeout(() => {
                    $('#new-success-box').hide(); // Ensure correct ID
                }, 3000);
            },
